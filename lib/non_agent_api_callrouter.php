...
### custom setters burk@5gigahertz.com (b5)
...
if (isset($_GET["external_extension"]))			{$external_extension=$_GET["external_extension"];}
	elseif (isset($_POST["external_extension"]))	{$external_extension=$_POST["external_extension"];}
if (isset($_GET["ra_status"]))			{$ra_status=$_GET["ra_status"];}
	elseif (isset($_POST["ra_status"]))	{$ra_status=$_POST["ra_status"];}
if (isset($_GET["group_id"]))			{$group_id=$_GET["group_id"];}
	elseif (isset($_POST["group_id"]))	{$group_name=$_POST["group_id"];}
...
if (isset($_GET["blocked_states"]))			{$blocked_states=$_GET["blocked_states"];}
	elseif (isset($_POST["blocked_states"]))	{$blocked_states=$_POST["blocked_states"];}
...
###
### add_remote_agent - creates remote agent from existing user burk@5gigahertz.com (b5)
###

if ($function == 'add_remote_agent') {
  if (strlen($source) < 2) {
		$result = 'ERROR';
		$result_reason = "Invalid Source";
		echo "$result: $result_reason - $source\n";
    ...
    if ($allowed_user < 1) {
			$result = 'ERROR';
			$result_reason = "add_remote_agent USER DOES NOT HAVE PERMISSION TO MODIFY USERS";
			$data = "$allowed_user";
			echo "$result: $result_reason: |$user|$data\n";
			api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
			exit;
    } else {
      # Let's check for required fields
      if ( (strlen($agent_user)<2) or (strlen($number_of_lines)<1) or (strlen($server_ip)<1) or (strlen($external_extension)<1) or (strlen($ra_status)<1) or (strlen($campaign_id)<1) ) {
				$result = 'ERROR';
				$result_reason = "add_remote_agent YOU MUST USE ALL REQUIRED FIELDS";
				$data = "$agent_user|$number_of_lines|$server_ip|$external_extension|$ra_status|$campaign_id";
				echo "$result: $result_reason: |$user|$data\n";
				api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
				exit;
      } else {
    		$stmt="SELECT count(*) from vicidial_remote_agents where server_ip='$server_ip' and user_start='$agent_user';";
    		$rslt=mysql_to_mysqli($stmt, $link);
    		$row=mysqli_fetch_row($rslt);
        if ($row[0] > 0) {
    			$result = 'ERROR';
    			$result_reason = "add_remote_agent THERE IS ALREADY A REMOTE AGENTS ENTRY STARTING WITH THIS USER ID";
    			$data = "$agent_user";
    			echo "$result: $result_reason: |$user|$data\n";
    			api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
        } else {
    			...
    			if ($row[0] < 1) {
      			$result = 'ERROR';
      			$result_reason = "add_remote_agent YOU MUST USE A VALID USER AS THE AGENT_USER FOR REMOTE AGENTS";
    			} else {
            # The real work of adding the remote agent:
    				if (strlen($in_groups) > 0) {
    					$in_groups = preg_replace("/\|/"," ",$in_groups);
    					$in_groups = " ".$in_groups." -";
    				}

    				$stmt="INSERT INTO vicidial_remote_agents (user_start,number_of_lines,server_ip,conf_exten,status,campaign_id,closer_campaigns) values('$agent_user','$number_of_lines','$server_ip','$external_extension','$ra_status','$campaign_id','$in_groups');";
    				$rslt=mysql_to_mysqli($stmt, $link);
    				...
    				$stmt="INSERT INTO vicidial_admin_log set event_date='$SQLdate', user='$PHP_AUTH_USER', ip_address='$ip', event_section='REMOTEAGENTS', event_type='ADD', record_id='$agent_user', event_code='ADMIN ADD REMOTE AGENT', event_sql=\"$SQL_log\", event_notes='';";
    				if ($DB) {echo "|$stmt|\n";}
    				$rslt=mysql_to_mysqli($stmt, $link);

    				$result = 'SUCCESS';
    				$result_reason = "add_remote_agent REMOTE AGENT HAS BEEN ADDED";
    				$data = "$agent_user|$number_of_lines|$server_ip|$external_extension|$ra_status|$campaign_id";
    				echo "$result: $result_reason - $user|$data\n";
    				api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
            exit;
...
###
### update_remote_agent - creates remote agent from existing user burk@5gigahertz.com
###
    ...
    if ($allowed_user < 1) {
      ...
      if ( (strlen($agent_user)<2) or (strlen($number_of_lines)<1) or (strlen($server_ip)<1) or (strlen($external_extension)<1) or (strlen($ra_status)<1) or (strlen($campaign_id)<1) ) {
        ...
        if ($row[0] > 0) {
  				...
  				$stmt="UPDATE vicidial_remote_agents SET number_of_lines='$number_of_lines', server_ip='$server_ip', conf_exten='$external_extension', status='$ra_status', closer_campaigns='$in_groups' WHERE user_start='$agent_user' LIMIT 1;";
  				$rslt=mysql_to_mysqli($stmt, $link);
      		$result = 'SUCCESS';
      		$result_reason = "update_remote_agent REMOTE AGENT UPDATED";
  				$data = "$agent_user|$number_of_lines|$server_ip|$external_extension|$ra_status";
  				echo "$result: $result_reason: |$user|$data\n";
      		api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
          exit;
        }
...
###
### add_ingroup adds a new callrouter ingroup burk@5gigahertz.com (b5)
###

if ($function == 'add_ingroup') {
  if (strlen($source) < 2) {
		$result = 'ERROR';
		$result_reason = "Invalid Source";
		echo "$result: $result_reason - $source\n";
    ...
    # Let's check for required fields
    if ( (strlen($group_id)<2) or (strlen($group_name)<1) ) {
			$result = 'ERROR';
			$result_reason = "add_ingroup YOU MUST USE ALL REQUIRED FIELDS";
      ...
      $group_id = preg_replace("/\-/",'',$group_id);
  		...
  		if ($row[0] > 0) {
  		  $result = 'ERROR';
        $result_reason = "GROUP NOT ADDED - there is already a group in the system with this ID";
  			$data = "$group_id";
  			echo "$result: $result_reason: |$user|$data\n";
  		} else {
  			...
  			if ($row[0] > 0) {
  			  $result = 'ERROR';
          $result_reason = "GROUP NOT ADDED - there is already a campaign in the system with this ID";
    			$data = "$group_id";
    			echo "$result: $result_reason: |$user|$data\n";
  			} else {
  				$stmt="INSERT INTO vicidial_inbound_groups (group_id,group_name,group_color,areacode_filter,active,web_form_address,voicemail_ext,next_agent_call,fronter_display,ingroup_script,get_call_launch,web_form_address_two,start_call_url,dispo_call_url,add_lead_url,na_call_url,user_group,group_handling,web_form_address_three) values('$group_id','$group_name','#1da3ed','DROP_ONLY','Y','" . mysqli_real_escape_string($link, '') . "','','longest_wait_time','Y','','NONE','','$start_call_url','$start_call_url','','$start_call_url','CallRouter','PHONE','');";
  				$rslt=mysql_to_mysqli($stmt, $link);
  				...
  				$rslt=mysql_to_mysqli($stmtB, $link);
					...
					$groups_value='';
          $stmtC="SELECT closer_campaigns from vicidial_campaigns where campaign_id='VTRouter';";
					$rslt=mysql_to_mysqli($stmtC, $link);
					$row=mysqli_fetch_row($rslt);
					$Mcloser_campaigns = $row[0];
					$Mcloser_campaigns = preg_replace("/ -$/","",$Mcloser_campaigns);
          $groups_value = $Mcloser_campaigns;
          $groups_value .= " $group_id";
          
          if (strlen($groups_value)>2) {$groups_value .= " -";}
                  
          $stmtD="UPDATE vicidial_campaigns SET closer_campaigns='$groups_value' WHERE campaign_id='VTRouter';";
  				$rslt=mysql_to_mysqli($stmtD, $link);
					...
					$SQL_log = preg_replace('/;/', '', $SQL_log);
					$SQL_log = addslashes($SQL_log);
					$stmt="INSERT INTO vicidial_admin_log set event_date='$SQLdate', user='$user', ip_address='$ip', event_section='INGROUPS', event_type='ADD', record_id='$group_id', event_code='ADMIN ADD INBOUND GROUP', event_sql=\"$SQL_log\", event_notes='';";
					$rslt=mysql_to_mysqli($stmt, $link);
          
  			  $result = 'SUCCESS';
          $result_reason = "GROUP ADDED";
          $data = "$group_id";
          echo "$result: $result_reason: |$user|$data|$groups_value\n"; api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
          exit;
        }
			}
		}
...
###
### delete_ingroup burk@5gigahertz.com (b5)
###

if ($function == 'delete_ingroup') {
  if (strlen($source) < 2) {
		$result = 'ERROR';
		$result_reason = "Invalid Source";
		echo "$result: $result_reason - $source\n";
    ...
    if (strlen($group_id)<2) {
			$result = 'ERROR';
			$result_reason = "delete_ingroup YOU MUST USE ALL REQUIRED FIELDS";
			$data = "$group_id";
      echo "$result: $result_reason: |$user|$data\n";
    } else {
  		...
  		if ($row[0] < 1) {
  			$result = 'ERROR';
  			$result_reason = "delete_ingroup INGROUP NOT FOUND";
  			$data = "$group_id";
        echo "$result: $result_reason: |$user|$data\n";
  		} else {
  			...
  			$result = 'SUCCESS';
  			$result_reason = "delete_ingroup INGROUP HAS BEEN REMOVED";
...
###
### add_did - adds a new inbound DID burk@5gigahertz.com (b5)
###
  ...
  if(strlen($source)<2) {
    ...
    if ( (strlen($did_pattern)<2) or (strlen($did_pattern)>50) ) {
      ...
      if ($did_exists > 1) {
        ...
        $user_group = 'CallRouter';
        
        # Create the filter phone group for this DID.
        $filter_phone_group_id = $did_pattern;
        $filter_phone_group_name = $did_pattern;
        $filter_phone_group_description = $did_description;
				$stmtFPG="INSERT INTO vicidial_filter_phone_groups (filter_phone_group_id,filter_phone_group_name,filter_phone_group_description,user_group) values('$filter_phone_group_id','$filter_phone_group_name','$filter_phone_group_description','$user_group');";
				$rslt=mysql_to_mysqli($stmtFPG, $link);
        
        # Add the DID if it does not exist.
  			$stmtA="INSERT INTO vicidial_inbound_dids (did_pattern,did_description,user_group,list_id) values('$did_pattern','$did_description','$user_group','909');";
  			$rslt=mysql_to_mysqli($stmtA, $link);
  			$result = 'SUCCESS';
...
###
### update_filter_phone_group - burk@5gigahertz.com (b5)
###

if ($function == 'update_filter_phone_group') {
  if(strlen($source)<2) {
		$result = 'ERROR';
		$result_reason = "Invalid Source";
		echo "$result: $result_reason - $source\n";
    ...
    # Reset vicidial_filter_phone_numbers:
		$stmt="DELETE FROM vicidial_filter_phone_numbers WHERE filter_phone_group_id='$filter_phone_group_id';";
		$rslt=mysql_to_mysqli($stmt, $link);
    
    # Load current filterd states:
    $fpg_states = explode(',', $blocked_states);
    $fpg_states_ct = count($fpg_states);
    $k = 0;
    while ($k < $fpg_states_ct) {
      $fpg_todo = $fpg_states[$k];
      $k++;
      $stmt="SELECT areacode FROM vicidial_phone_codes WHERE country_code=1 AND state='$fpg_todo';";
      $rslt=mysql_to_mysqli($stmt, $link);
      $areacodes_to_append = mysqli_fetch_row($rslt);
      $areacodes_to_append_ct = count($areacodes_to_append);
      $l = 0;
      while ($l < $areacodes_to_append_ct) {
        $areacode_to_add = $areacodes_to_append[$l] . 'XXXXXXX';
				$stmt="INSERT INTO vicidial_filter_phone_numbers (phone_number,filter_phone_group_id) values('$areacode_to_add','$filter_phone_group_id');";
				$rslt=mysql_to_mysqli($stmt, $link);
        $l++;
        
  			$result = 'SUCCESS';
  			$result_reason = "update_filter_phone_group BLOCKED NUMBER HAS BEEN ADDED";
  			$data = "$areacode_to_add|$filter_phone_group_id";
  			echo "$result: $result_reason - $user|$data\n";
  			api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
      }
    }
...
### all_agent_status - exports all incall agents

if ($function == 'all_agents_status') {
	$caller_ids = [];
  $length_of_call_in_secs = [];
  $did_called = [];
	$stmt="SELECT vicidial_live_agents.user,vicidial_live_agents.calls_today,vicidial_auto_calls.phone_number, (NOW() - vicidial_auto_calls.call_time), vicidial_list.vendor_lead_code FROM vicidial_live_agents,vicidial_auto_calls,vicidial_list WHERE vicidial_live_agents.lead_id = vicidial_auto_calls.lead_id AND vicidial_live_agents.lead_id = vicidial_list.lead_id;";
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "$stmt\n";}
  ...
  if ($talking_to_print > 0) {
    ...
    while ($i < $talking_to_print) {
      $row=mysqli_fetch_row($rslt);
      $Auser[$i] = $row[0];
      $Acalls_today[$i] =	$row[1];
      $caller_ids[$i] = $row[2];
      $length_of_call_in_secs[$i] = $row[3];
      $did_called[$i] = $row[4];
      $i++;
    }
  }
  ...
  if ($talking_to_print > 0) {
    $i=0;
    while ($i < $talking_to_print) {
      $json_data[] = [$Auser[$i],$Acalls_today[$i],$caller_ids[$i],$length_of_call_in_secs[$i],$did_called[$i]];
      #echo "$Auser[$i],$Acalls_today[$i],$caller_ids[$i]<br />";
      $i++;
    }
